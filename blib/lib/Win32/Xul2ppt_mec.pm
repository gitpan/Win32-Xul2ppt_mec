...
to get a mini-ppt files Image-Magic intaller also prerequired.
...
The attributes of the Xul2ppt_mec are described below, in the I<Attributes>
section.
...
recommended. Two command lines provided by Imagic-Magic are invoking: "convert -crop +0+25 $file $file" and 
...
Automacally inserts pictures in your ppt files. Before it, powerpoint should be starting and not minimized!
...
=item * $obj->{'range'} = [$start, $end]: 

Set the index of Xul pictures start and end shooting
...
=item * -r See L<$obj->{'range'}>

=item * -i See L<$obj->{'interval'}>

=item * -d See L<$obj->{'dir'}>

=item * -n See L$obj->{'name'}>
